...
function captureScreenshotBase64(): string | null {
  try {
    ...
    if (buffer && buffer.length > 100) {
      // Try to compress via Python (75% smaller = faster LLM calls)
      try {
        const compress = Bun.spawnSync(["python3", "-c", `
import sys, base64
...
`], {
        ...
        const compressed = compress.stdout.toString();
        if (compressed.length > 100) {
          console.log(`Screenshot: ${buffer.length}B PNG -> ${Math.round(compressed.length * 3/4)}B JPEG`);
          return compressed;
        }
      } catch {}
      // Fallback: raw PNG
